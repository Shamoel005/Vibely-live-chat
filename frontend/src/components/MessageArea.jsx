...
// <div className='w-full h-[100px] bg-[#663399] rounded-b-[30px] shadow-gray-400 shadow-lg gap-[20px] flex items-center px-[20px] '>
...
//      <form className='w-[95%] lg:w-[70%] h-[60px] bg-[#663399] shadow-gray-400 shadow-lg rounded-full flex items-center gap-[20px] px-[20px] relative' onSubmit={handleSendMessage}>
...
//     <h1 className='text-gray-700 font-bold text-[50px]'>Welcome to Vibely</h1>
...
//             {messages && messages.map((mess) => (
//               mess.sender._id === userData._id ? ( // FIXED: Compare mess.sender._id with userData._id
...
function MessageArea() {
  ...
  const handleMessageDelete = (deletedMessageId) => {
    dispatch(setMessages(messages.filter(msg => msg._id !== deletedMessageId)));
  };
  ...
  useEffect(() => {
    ...
    socket?.on("messageDeleted", (data) => {
      dispatch(setMessages(messages.filter(msg => msg._id !== data.messageId)));
    });
